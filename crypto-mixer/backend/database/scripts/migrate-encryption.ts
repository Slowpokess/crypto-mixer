...
import { DataEncryptionManager } from '../utils/DataEncryption';
import { ExistingDataEncryptionMigrator } from '../utils/EncryptExistingData';
...
class EncryptionMigrationCLI {
  ...
  private migrator!: ExistingDataEncryptionMigrator;
  ...
  async migrate(options: { dryRun: boolean; batchSize: number; table?: string }): Promise<void> {
    console.log('🔒 Starting sensitive data encryption migration...');
    console.log('Options:', options);

    try {
      this.migrator = new ExistingDataEncryptionMigrator(
        this.sequelize,
  ...
  async status(): Promise<void> {
    ...
    try {
      this.migrator = new ExistingDataEncryptionMigrator(this.sequelize, this.encryptionManager);
      ...
      for (const [tableName, tableProgress] of Object.entries(status.progress)) {
        const table = tableProgress as any;
        ...
        for (const [fieldName, fieldProgress] of Object.entries(table.fields)) {
          const field = fieldProgress as any;
  ...
  async rollback(options: { dryRun: boolean; table?: string }): Promise<void> {
  ...
  async testEncryption(): Promise<void> {
    ...
    try {
      ...
      const encrypted = await this.encryptionManager.encryptSensitiveData(
        testData,
