...
export class MigrationManager {
  ...
  private migrationTableName: string = 'sequelize_migrations';
   ...
   */
  private calculateChecksum(migration: MigrationFile): string {
  ...
  private async recordMigration(
    ...
    await this.sequelize.query(
      'INSERT INTO ?? (id, filename, executed_at, execution_time_ms, checksum) VALUES (?, ?, ?, ?, ?)',
      {
        replacements: [
   ...
   */
  private async unrecordMigration(migrationId: string): Promise<void> {
    // БЕЗОПАСНОСТЬ: Валидация имени таблицы
    this.validateTableName(this.migrationTableName);
   ...
   */
  async create(name: string, template: 'table' | 'index' | 'custom' = 'custom'): Promise<string> {
   ...
   */
  private generateTableTemplate(name: string): string {
...
`;
   ...
   */
  private generateCustomTemplate(name: string): string {
    return `import { QueryInterface, DataTypes } from 'sequelize';
...
`;
  ...
  private validateMigrationsPath(migrationsPath: string): void {
    ...
    const normalizedPath = path.normalize(migrationsPath);
    ...
    const stat = fs.statSync(migrationsPath);
    if (!stat.isDirectory()) {
   ...
   */
  private validateAndBuildFilePath(filename: string): string {
    ...
    if (!normalizedPath.startsWith(path.normalize(this.migrationsPath))) {
   ...
   */
  private safeRequire(filepath: string): any {
    try {
   ...
   */
  private validateMigrationStructure(migration: any, filename: string): void {
