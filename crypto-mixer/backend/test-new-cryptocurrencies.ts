import { LitecoinCoreClient, LitecoinCoreConfig } from './blockchain/nodes/LitecoinCoreClient';
import { DashCoreClient, DashCoreConfig } from './blockchain/nodes/DashCoreClient';
import { ZcashClient, ZcashConfig } from './blockchain/nodes/ZcashClient';
 ...
 */
async function testLitecoinClient(): Promise<boolean> {
  console.log('\n🚀 Тестирование Litecoin Core Client...');
  
  try {
    const ltcClient = new LitecoinCoreClient(testConfigs.litecoin);
    ...
    console.log(`   ❌ Ошибка тестирования Litecoin: ${(error as Error).message}`);
    return false;
  }
}
 ...
 */
async function testDashClient(): Promise<boolean> {
  console.log('\n🚀 Тестирование Dash Core Client...');
  
  try {
    const dashClient = new DashCoreClient(testConfigs.dash);
    ...
    console.log(`   ❌ Ошибка тестирования Dash: ${(error as Error).message}`);
    return false;
  }
}
 ...
 */
async function testZcashClient(): Promise<boolean> {
  console.log('\n🚀 Тестирование Zcash Client...');
  
  try {
    const zcashClient = new ZcashClient(testConfigs.zcash);
    ...
    console.log(`   ❌ Ошибка тестирования Zcash: ${(error as Error).message}`);
    return false;
  }
}
 ...
 */
async function testBlockchainManager(): Promise<boolean> {
  ...
  try {
    ...
    console.log(`   ❌ Ошибка тестирования BlockchainManager: ${(error as Error).message}`);
    return false;
  }
}
 ...
 */
function testAddressValidation(): boolean {
  console.log('\n🚀 Тестирование валидации адресов...');
  ...
  const validateAddress = (address: string, currency: string): boolean => {
    ...
    if (!currencyPatterns) return false;
