...
function checkTestFiles() {
  ...
  for (const testFile of allTests) {
    const fullPath = resolve(testDir, testFile);
...
function runTests(testFiles: string[], testType: string) {
  ...
  const vitestCommand = [
    ...
    `--config ${resolve(testDir, 'vitest.config.ts')}`,
    testFiles.map(f => resolve(testDir, f)).join(' ')
  ].filter(Boolean).join(' ');
...
function generateTestReport() {
  ...
  try {
    const fs = require('fs');
    const reportPath = resolve(testDir, 'test-report.json');
...
function printSummary(results: { unit?: boolean; integration?: boolean; linting?: boolean }) {
  ...
  log(`\nОбщий результат: ${passedTests}/${totalTests} успешно`, 
...
async function main() {
  ...
  if (isUnitOnly) log('  - Только unit тесты');
  if (isIntegrationOnly) log('  - Только интеграционные тесты');
  if (withCoverage) log('  - С анализом покрытия кода');
  if (watchMode) log('  - В режиме наблюдения');
  if (verbose) log('  - Подробный вывод');

  // Предварительные проверки
  checkDependencies();
  checkTestFiles();

  const results: { unit?: boolean; integration?: boolean; linting?: boolean } = {};
