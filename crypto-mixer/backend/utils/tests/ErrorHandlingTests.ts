...
import { initializeLoggingSystems } from '../logger';
...
export class ErrorHandlingTests {
  ...
  private async initializeSystems(): Promise<void> {
    await this.runTest('Инициализация систем', async () => {
      ...
      initializeErrorHandler(this.logger, {
        enabled: true,
   ...
   */
  private async runErrorTypeTests(): Promise<void> {
    ...
    await this.runTest('Создание кастомной ошибки', async () => {
      const error = new MixingError(
        'Mixing pool is full',
        ErrorCode.MIXING_POOL_FULL,
        { component: 'mixer', operation: 'joinPool', additionalInfo: { amount: 1000 } },
   ...
   */
  private async runErrorHandlerTests(): Promise<void> {
    ...
    await this.runTest('Обработка простой ошибки', async () => {
      ...
      const error = new DatabaseError(
        ...
        { component: 'test', operation: 'testQuery' }
      );
   ...
   */
  private async runRetryMechanismTests(): Promise<void> {
    console.log('\n🔄 Тестирование Retry механизмов...');

    await this.runTest('Успешная операция без retry', async () => {
      const result = await withRetry(
        async () => 'success',
        { component: 'test', operation: 'successOperation' }
      );
    ...
    await this.runTest('Операция с retry после ошибок', async () => {
      ...
      const result = await withRetry(
        async () => {
          attemptCount++;
          if (attemptCount < 3) {
            throw new DatabaseError('Temporary failure', ErrorCode.QUERY_FAILED);
          }
          return 'success after retry';
        },
        { component: 'test', operation: 'retryOperation' },
    ...
    await this.runTest('Операция с исчерпанием retry', async () => {
      const result = await withRetry(
        async () => {
          throw new DatabaseError('Persistent failure', ErrorCode.QUERY_FAILED);
        },
        { component: 'test', operation: 'failOperation' },
    ...
    await this.runTest('Невосстанавливаемая ошибка без retry', async () => {
      const result = await withRetry(
        async () => {
          throw new SecurityError('Security violation', ErrorCode.SECURITY_VIOLATION);
        },
        { component: 'test', operation: 'securityFailure' },
        { maxRetries: 3 }
      );
  ...
  private async runAuditLoggingTests(): Promise<void> {
    ...
    await this.runTest('Логирование ошибки в audit', async () => {
      const auditLogger = getAuditLogger();
      const error = new AuthenticationError(
        'Test auth error for audit',
        ErrorCode.INVALID_CREDENTIALS,
        { component: 'auth', operation: 'testAuth' }
      );
   ...
   */
  private async runPerformanceMonitoringTests(): Promise<void> {
   ...
   */
  private async runAlertSystemTests(): Promise<void> {
    ...
    await this.runTest('Обработка ошибки для алерта', async () => {
      const alertManager = getAlertManager();
      const error = new SecurityError(
        'Test critical error for alert',
        ErrorCode.SECURITY_VIOLATION,
        { component: 'security', operation: 'testAlert' }
      );
        ...
        async (span) => {
          return withRetry(
            async () => {
              attempts++;
              if (attempts < 2) {
                throw new DatabaseError('Retry test error', ErrorCode.QUERY_FAILED);
              }
              return 'success';
            },
            {
