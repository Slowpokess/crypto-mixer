...
exports.demonstrateBackupSystem = demonstrateBackupSystem;
exports.demonstrateConfigurations = demonstrateConfigurations;
exports.demonstrateBestPractices = demonstrateBestPractices;
...
async function demonstrateBackupSystem() {
    ...
    try {
        ...
        const config = backup_1.BackupUtils.generateRecommendedConfig('development');
        // Валидация конфигурации
        const validation = backup_1.BackupUtils.validateConfig(config);
        if (!validation.valid) {
        ...
        const backupSystem = (0, backup_1.createIntegratedBackupSystem)(config);
        ...
        console.log('⏱️ Время работы:', backup_1.BackupUtils.formatDuration(systemStatus.uptime));
        ...
        console.log('✅ Backup создан:', {
            ...
            duration: backup_1.BackupUtils.formatDuration(backupReport.duration),
            size: backup_1.BackupUtils.formatSize(backupReport.totalSize),
        ...
        const monitoring = backupSystem.monitoring;
        if (monitoring) {
        ...
        const drManager = backupSystem.drManager;
        if (drManager) {
            ...
            try {
                const recoveryExecution = await backupSystem.performRecovery('application_crash_recovery', { dryRun: true });
                console.log('✅ Тестовое восстановление выполнено:', {
                    id: recoveryExecution.id,
                    status: recoveryExecution.status,
                    duration: backup_1.BackupUtils.formatDuration(recoveryExecution.totalDuration || 0),
        ...
        const dashboard = backupSystem.dashboard;
        if (dashboard) {
        ...
        console.log(`  • 1024 bytes = ${backup_1.BackupUtils.formatSize(1024)}`);
        console.log(`  • 1048576 bytes = ${backup_1.BackupUtils.formatSize(1048576)}`);
        console.log(`  • 1073741824 bytes = ${backup_1.BackupUtils.formatSize(1073741824)}`);
        console.log('⏱️ Форматирование времени:');
        console.log(`  • 30 секунд = ${backup_1.BackupUtils.formatDuration(30)}`);
        console.log(`  • 3600 секунд = ${backup_1.BackupUtils.formatDuration(3600)}`);
        console.log(`  • 86400 секунд = ${backup_1.BackupUtils.formatDuration(86400)}`);
        console.log('📅 Следующий backup:');
        const nextBackup = backup_1.BackupUtils.getNextBackupTime('0 2 * * *');
        console.log(`  • Запланирован на: ${nextBackup.toLocaleString()}`);
        console.log('🏷️ Доступные константы:');
        console.log(`  • Типы backup: ${Object.values(backup_1.BACKUP_CONSTANTS.BACKUP_TYPES).join(', ')}`);
        console.log(`  • Статусы: ${Object.values(backup_1.BACKUP_CONSTANTS.BACKUP_STATUSES).join(', ')}`);
        console.log(`  • Приоритеты: ${Object.values(backup_1.BACKUP_CONSTANTS.COMPONENT_PRIORITIES).join(', ')}`);
        console.log();
        ...
        console.log(`  • Время работы: ${backup_1.BackupUtils.formatDuration(finalStatus.uptime)}`);
...
async function demonstrateConfigurations() {
    ...
    const devConfig = backup_1.DEFAULT_CONFIGS.DEVELOPMENT;
    ...
    console.log('\n🏭 Production конфигурация:');
    const prodConfig = backup_1.DEFAULT_CONFIGS.PRODUCTION;
    ...
    console.log('\n🧪 Testing конфигурация:');
    const testConfig = backup_1.DEFAULT_CONFIGS.TESTING;
...
async function main() {
    try {
        ...
        const { initializeLoggingSystems } = await Promise.resolve().then(() => __importStar(require('../utils/logger')));
...
//# sourceMappingURL=backup-demo.js.map