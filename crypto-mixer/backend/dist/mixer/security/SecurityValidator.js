 ...
 */
class SecurityValidator extends events_1.EventEmitter {
    ...
    _analyzeTransactionVelocity(userActivity) {
        ...
        const recentTransactions = userActivity.filter(tx => now - new Date(tx.created_at).getTime() < oneHour);
    ...
    _analyzeAmountPatterns(userActivity) {
        ...
        const isStructuring = amounts.every(amt => amt < avgAmount * 1.2 && amt > avgAmount * 0.8);
    ...
    _analyzeAddressPatterns(userActivity) {
        ...
        const allOutputs = userActivity.flatMap(tx => (tx.output_addresses ? JSON.parse(tx.output_addresses) : []).map((out) => out.address));
    ...
    _isRoundAmount(amount) {
        ...
        return roundThresholds.some(threshold => Math.abs(amount - threshold) < threshold * 0.01);
    }
