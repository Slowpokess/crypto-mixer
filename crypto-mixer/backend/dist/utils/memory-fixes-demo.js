...
async function demonstrateMemoryFixes() {
    ...
    let stats = MemoryManager_1.memoryManager.getStats();
    ...
    const testMap = MemoryManager_1.memoryManager.createBoundedMap('demo-collection', {
        maxSize: 100,
    ...
    for (let i = 0; i < 5; i++) {
        MemoryManager_1.memoryManager.createTimer(`demo-timer-${i}`, () => {
            console.log(`   Timer ${i} executed`);
        }, 5000, 'interval', `Demo timer ${i}`);
    }
    stats = MemoryManager_1.memoryManager.getStats();
    ...
    // Clear demo timers
    for (let i = 0; i < 5; i++) {
        MemoryManager_1.memoryManager.clearTimer(`demo-timer-${i}`);
    }
    ...
    // Show memory health
    const health = MemoryMonitoring_1.memoryMonitoring.getCurrentStatus();
    if (health) {
...
//# sourceMappingURL=memory-fixes-demo.js.map