...
async function testAddressGeneration(): Promise<boolean> {
  ...
  try {
    ...
    const ltcClient = new LitecoinCoreClient(ltcConfig);
    ...
    const dashConfig: DashCoreConfig = {
      host: 'localhost',
      port: 9998,
      username: 'test',
      password: 'test'
    ...
    const dashClient = new DashCoreClient(dashConfig);
    ...
    const zcashClient = new ZcashClient(zcashConfig);
    ...
    console.log(`   ❌ Ошибка создания адресов: ${(error as Error).message}`);
    return false;
  }
}
 ...
 */
function testCurrencyConfigs(): boolean {
